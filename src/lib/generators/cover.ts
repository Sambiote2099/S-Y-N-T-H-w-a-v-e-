...
function fitText(ctx: Ctx, text: string, maxWidth: number, maxLines: number, startSize = 42, minSize = 18) {
  ...
  while (fontSize > minSize) {
    ctx.font = `bold ${fontSize}px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
...
function drawTextOverlay(ctx: Ctx, title: string, artist: string, plan: TextPlacementPlan, isLight = false) {
  ...
  if (plan.placement === "bottomBand" || plan.placement === "topBand") {
    ...
    if (atBottom) {
      ...
      ctx.font = `600 20px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
      ctx.shadowBlur = 8;
    ...
    ctx.font = `bold 19px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    const cardW = 220;
    ...
    ctx.font = `600 14px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    ...
    ctx.shadowColor = glow;
    ctx.shadowBlur = 10;
    ctx.font = `bold 28px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    ctx.fillText(truncate(title, 24).toUpperCase(), 0, 0);
    ctx.restore();

    ctx.textAlign = side === "left" ? "left" : "right";
    ctx.font = `600 16px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    ...
    ctx.shadowBlur = 8;
    ctx.font = `600 15px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    ctx.fillText(truncate(title, 30).toUpperCase(), x, y);
    ctx.font = `400 12px "NotoSans", "Helvetica Neue", Arial, sans-serif`;
    ctx.fillStyle = inkFaint;
